...
public class UpdateChecker implements Listener {
    ...
    public void fetch() {
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                plugin.getStartUpLogger().message("§4§l§nFailed to check for updates on spigot.");
                return;
            }
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                plugin.getStartUpLogger().message("§3§lAn update for AdminPanel (v§f§l" + getSpigotVersion() + "§3§l) is available at:");
                plugin.getStartUpLogger().message("§a§lhttps://www.spigotmc.org/resources/servermanager-an-mc-integrated-admin-panel-german." + RESOURCE_ID + "/");
    ...
    public void onJoin(PlayerJoinEvent e) {
        if (e.getPlayer().hasPermission("AdminPanel.updatenotify") && hasUpdateAvailable()) {
            Msg.msg(e.getPlayer(),
