...
public class AdminPanelUtils {
    ...
    private static AdminPanelUtils instance;

    public AdminPanelUtils() {
    ...
    public static String arrayToString(String[] array) {
        ...
        for (String s : array) {
            if (!s.isEmpty())
                sbOut.append(AdminPanelUtils.format(null, s, AdminPanelMain.getPrefix())).append(" ");
        }
    ...
    public static void serverStop(int timeBeforeStop) {
        ...
        Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&a+---------------------------------------------------+", AdminPanelMain.getPrefix()));
        Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&r[&4&lWARNING&r] " + "&c&lThe server will now shut down and all players will be kicked!", AdminPanelMain.getPrefix()));
        Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&a+---------------------------------------------------+", AdminPanelMain.getPrefix()));
        ...
        for (int i = 6; i > 0; i--) {
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                clearChat(100, false, null);
                if (secondsLeft <= 2) {
                    Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&4Serverstop in: &c&l" + secondsLeft, AdminPanelMain.getPrefix()));
                } else if (secondsLeft <= 4) {
                    Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&6Serverstop in: &b&l" + secondsLeft, AdminPanelMain.getPrefix()));
                } else {
                    Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&aServerstop in: &a&l" + secondsLeft, AdminPanelMain.getPrefix()));
                }
            }, (long) (6 - i) * 20);
        }
        ...
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            clearChat(100, false, null);
            Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&a+---------------------------------------------------+", AdminPanelMain.getPrefix()));
            Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&r[&4&lWARNING&r] " + "&c&lServer Stop initiated!", AdminPanelMain.getPrefix()));
            Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&a+---------------------------------------------------+", AdminPanelMain.getPrefix()));
        }, 6 * 20);

        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            for (Player p2 : Bukkit.getServer().getOnlinePlayers()) {
                p2.kickPlayer(AdminPanelUtils.format(null, "&4&lThe server is now shutting down!", AdminPanelMain.getPrefix()));
            }
            Bukkit.shutdown();
        }, (6 * 20) + timeBeforeStop);
    }
    ...
    public static void serverRestart(int timeBeforeRestart) {
        ...
        clearChat(100, false, null);
        Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&a+---------------------------------------------------+", AdminPanelMain.getPrefix()));
        Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&r[&4&lWARNING&r] " + "&c&lThe server is about to restart!", AdminPanelMain.getPrefix()));
        Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&a+---------------------------------------------------+", AdminPanelMain.getPrefix()));
        ...
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            for (int i = 6; i > 0; i--) {
                ...
                Bukkit.getScheduler().runTaskLater(plugin, () -> {
                    clearChat(100, false, null);
                    if (secondsLeft <= 2) {
                        Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&4Serverrestart in: &c&l" + secondsLeft, AdminPanelMain.getPrefix()));
                    } else if (secondsLeft <= 4) {
                        Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&6Serverrestart in: &b&l" + secondsLeft, AdminPanelMain.getPrefix()));
                    } else {
                        Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&aServerrestart in: &a&l" + secondsLeft, AdminPanelMain.getPrefix()));
                    }
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                clearChat(100, false, null);
                Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&a+---------------------------------------------------+", AdminPanelMain.getPrefix()));
                Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&r[&4&lWARNING&r] " + "&c&lServer Restart initiated!", AdminPanelMain.getPrefix()));
                Bukkit.broadcastMessage(AdminPanelUtils.format(null, "&a+---------------------------------------------------+", AdminPanelMain.getPrefix()));
            }, 6L * 20);

            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                Bukkit.getOnlinePlayers().forEach(p2 -> p2.kickPlayer(AdminPanelUtils.format(null, "&4&lThe server is now restarting!", AdminPanelMain.getPrefix())));
    ...
    public static AdminPanelUtils getInstance() {
        return instance;
    }
    ...
    public static Properties getProperties(File propertiesFile) {
        ...
        try {
            ...
            plugin.getFileLogger().writeToLog(Level.SEVERE, "Could not load properties file: " + propertiesFile.getName(), LogPrefixExtension.ERROR);
        }
        return properties;
    }
    ...
    public static void saveProperties(Properties properties, File propertiesFile) {
        try {
            ...
            plugin.getFileLogger().writeToLog(Level.SEVERE, "Could not save properties file: " + propertiesFile.getName(), LogPrefixExtension.ERROR);
        }
    }
    ...
    private void setInstance(AdminPanelUtils instance) {
        AdminPanelUtils.instance = instance;
    }
    ...
    private static void saveDataConfig() {
        try {
            ...
            plugin.getFileLogger().writeToLog(Level.SEVERE, "Could not save data.yml", LogPrefixExtension.ERROR);
        }
    }
    ...
    public static boolean sendMessageToWebhook(String message) {
        ...
        try {
            ...
            plugin.getFileLogger().writeToLog(Level.SEVERE, "Failed to send message to Discord webhook", LogPrefixExtension.ERROR);
    ...
    public static ItemStack createSkull(String headValue, String name, boolean isTexture) {
        ItemStack head = new ItemStack(AdminPanelUtils.legacyServer() ? Objects.requireNonNull(Material.matchMaterial("SKULL_ITEM")) : Material.PLAYER_HEAD, 1);
    ...
    public static InputStream getResource(@NotNull String filename) {
        try {
            URL url = AdminPanelUtils.class.getClassLoader().getResource(filename);
    ...
    public static void saveResource(File configFolder, @NotNull String resourcePath, boolean replace) {
        ...
        if (in == null) {
            throw new IllegalArgumentException("The embedded resource '" + resourcePath + "' cannot be found in " + AdminPanelUtils.class.getName() + "'s classloader");
        }
    ...
    public static void unzipFiles(String zipFilePath, String destDir, boolean replace) {
        ...
        try {
            ...
            plugin.getFileLogger().writeToLog(Level.SEVERE, "Failed to unzip files: " + e.getMessage(), LogPrefixExtension.ERROR);
        }
    }
    ...
    public static Menu getMenuByClassName(String className, Player player) {
        ...
        try {
            ...
            plugin.getFileLogger().writeToLog(Level.SEVERE, "Class not found: " + fullClassName, LogPrefixExtension.ERROR);
            ...
            plugin.getFileLogger().writeToLog(Level.SEVERE, "Error creating an instance: " + fullClassName, LogPrefixExtension.ERROR);
            plugin.getLogger().severe("Error creating an instance: " + fullClassName);
            return null; // You can modify the return value based on your needs
        } catch (InvocationTargetException | NoSuchMethodException e) {
            plugin.getFileLogger().writeToLog(Level.SEVERE, "Error invoking constructor: " + fullClassName, LogPrefixExtension.ERROR);
    ...
    public static void deleteMinecraftWorld(World world) {
        ...
        for (Player playerInWorld : world.getPlayers()) {
            playerInWorld.kickPlayer(AdminPanelUtils.chat("The World just got deleted!"));
        }
        new BukkitRunnable() {
            @Override
            public void run() {
