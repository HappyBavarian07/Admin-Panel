...
public class Utils {
	...
	public static void ban(final Player p, final String target, final String reason, final String sourcename) {
		try {
			...
			if(bannedPlayer.isBanned()) {
				...
				p.sendMessage("§cDu hast erfolgreich §a" + bannedPlayer.getName() + " §cfür §a" + Bukkit.getBanList(Type.NAME).getBanEntry(bannedPlayer.getName()).getReason().toString() + "§cgebannt!");
			}
	...
	@SuppressWarnings({"deprecation"})
	public static void unban(Player p, String target) {
		try {
			OfflinePlayer bannedPlayer = Bukkit.getOfflinePlayer(target);
			if(!bannedPlayer.isBanned()) {
				p.sendMessage("§cDer Spieler §a" + bannedPlayer.getName() + "§c ist nicht gebannt!");
			}
			if(bannedPlayer.isBanned()) {
				Bukkit.getBanList(Type.NAME).pardon(bannedPlayer.getName());
				p.sendMessage("§cDu hast erfolgreich §a" + bannedPlayer.getName() + "§c entgebannt!");
			}
		} catch (NullPointerException e) {
			p.sendMessage("§cDer Spieler ist nicht Online oder existiert nicht!");
		}
